...
echo "🧪 Загрузка фикстур (booking history db)..."
PGPASSWORD="${BOOKING_HISTORY_DB_PASSWORD}" psql -h "${BOOKING_HISTORY_DB_HOST}" -p "${BOOKING_HISTORY_DB_PORT}" -U "${BOOKING_HISTORY_DB_USER}" "${BOOKING_HISTORY_DB_NAME}" < init-fixtures-booking-history.sql
